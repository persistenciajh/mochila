...
public class MyMessageBox {
	protected static Logger log = Logger.getLogger(MyMessageBox.class);
	...
	public static Button show(String message, String title, Button[] buttons, String[] btnLabels, String icon,
			Button focus, EventListener listener, Map<String, String> params, String width, String height) {
		Map arg = new HashMap();
		...
		for (int j = 0; j < buttons.length; ++j) {
			...
			arg.put(buttons[j].id, Integer.valueOf(buttons[j].id)); // arg.put(buttons[j].stringId,
																	// Integer.valueOf(buttons[j].id));
		}
	...
	public static Button show(String message, String title, Button[] buttons, String icon, String width,
			String height) {
	...
	public static int show(String message, String title, int buttons, String icon, int focus,
		...
		return show(message, title, toButtonTypes(buttons), null, icon, (focus != 0) ? toButtonType(focus) : null,
				toButtonListener(listener), null, null, null).id;
	}

	public static int show(String message, String title, int buttons, String icon) {
		return show(message, title, toButtonTypes(buttons), null, icon, null, toButtonListener(null), null, null,
				null).id;
	}

	public static int show(String message, String title, int buttons, String icon, String width, String height) {
		return show(message, title, toButtonTypes(buttons), null, icon, null, toButtonListener(null), null, width,
				height).id;
	}
	...
	private static class ButtonListener implements SerializableEventListener<Messagebox.ClickEvent> {
		...
		public void onEvent(Messagebox.ClickEvent event) throws Exception {
			...
			this._listener.onEvent(
					new Event(event.getName(), event.getTarget(), Integer.valueOf((btn != null) ? btn.id : -1)));
		}
